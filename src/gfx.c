...
//	SDL_CreateWindowAndRenderer(0, 0, SDL_WINDOW_FULLSCREEN_DESKTOP, &sdlWindow, &sdlRenderer);
	...
	videomem=sdlSurface->pixels;
	darker=malloc(65536*sizeof(unsigned short));
	lighter=malloc(65536*sizeof(unsigned short));
	if(!darker || !lighter || !videomem) nomem(5);

	for(i=0;i<65536;++i)
	{
		SDL_GetRGB(i,sdlSurface->format,&r,&g,&b);
		darker[i]=SDL_MapRGB(sdlSurface->format,r*3>>2,g*3>>2,b*3>>2);
		if(r>128 && g>128 && b>128)
			lighter[i]=SDL_MapRGB(sdlSurface->format,(r+255)>>1,(g+255)>>1,(b+255)>>1);
		else
			lighter[i]=i;
	}
}
